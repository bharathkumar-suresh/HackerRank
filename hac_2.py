...
hackonacciStoredValues[0]=[[1,0,0],[0,1,0],[0,0,1]]
hackonacciStoredValues[1]=matrixMultiplyMatrix
...
def findNthElement(n,prevN):
    #print n,prevN
    if n<=3:
        return hackonacciSequence[n-1]
    prevN-=3
    n-=3
    if prevN<=0 or (n-prevN)<=0:
        prevN=0
    if n-prevN>0:
        multipliedMatrix=NPowerMatrixMultiply(n-prevN)

    #print prevN
    multipliedMatrix=multiplyMatrices(hackonacciStoredValues[prevN],multipliedMatrix)
    hackonacciStoredValues[n]=multipliedMatrix
...
for i in range(0,n):
    for j in range(i,n):
        product=((i+1)*(j+1))**2
        H[i][j]=findNthElement(product,previousProduct)
        previousProduct=((i+1)*(j))**2
        if H[i][j]%2==0:
            H[i][j]='X'
        else:
            H[i][j]='Y'
